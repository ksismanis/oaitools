...
orglist = [1002,1008,1018,1029,1030,1031,1045,1046,1049]
...
for org in orglist:
    ...
    parent_dir = "/data/data1/users/ksismanis/testoai/to.upscale/"
    parent_dir = parent_dir + str(org)
    # for key in urls:
    #     url = urls[key]
    upscaled_urls = []
    urls_checked = 0

    # full_path = os.path.join('/data/data1/users/ksismanis/testoai/', str(org)+'_list_latest.txt')
    ...
    print(str(org),len(upscaled_urls))
    full_path = os.path.join('/data/data1/users/ksismanis/testoai/', str(org)+'_list.txt')
    with open(full_path, 'w') as f:
        for url in upscaled_urls:
            # f.write(f"{url},")#\n
            f.write(f"<n>{url}</n>\n")#\n
        ...
        # retcode = subprocess.call(["wget","-nc","-P", str(org), url], stdout=FNULL, stderr=subprocess.STDOUT)
